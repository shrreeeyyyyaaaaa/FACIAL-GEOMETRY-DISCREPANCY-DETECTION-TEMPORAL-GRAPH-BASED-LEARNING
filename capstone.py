...
# Streamlit App
st.title("Facial Geometry Discrepancy Detection using Temporal Graph-Based Learning")
uploaded_file = st.file_uploader("Upload a video", type=["mp4", "avi"])

st.markdown("### **Model Performance on Test Data**")
st.markdown("- **Accuracy:** ~92%")
st.markdown("- **F1 Score:** 0.91")
st.markdown("- **ROC-AUC:** 0.94")
...
if uploaded_file:
    ...
    label = "Real" if avg_pred < 0.7 else "Fake"

    st.write(f"### Prediction: **{label}** ({avg_pred:.2f})")

    fig1, ax1 = plt.subplots()
    ax1.plot(preds, label='Sequence Predictions')
    ax1.axhline(0.5, color='red', linestyle='--', label='Threshold')
    ax1.set_title("Per-sequence Deepfake Prediction")
    ...
    # ROC Curve
    y_true = np.ones_like(preds) if label == "Real" else np.zeros_like(preds)
